...
quarto_metadata <- setNames( 
  nm = glue::glue("is_email_variant_{condition}"), 
  list(TRUE)
)
...
if (condition == "WARN") {
  ...
  warnings_table <- data.frame(step, desc,threshold,fails) |> 
    gt() |> 
    cols_label(
      step="Validation Step",
      desc="Description",
      fails="Fraction Failed", 
      threshold="Threshold"
    ) |>
    fmt_number(columns = c(fails, threshold),decimal = 2)
