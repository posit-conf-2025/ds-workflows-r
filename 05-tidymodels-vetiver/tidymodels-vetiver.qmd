...
Before starting this activity, we need to create and save a Posit Connect API key in our environment. This API key identifies you to Connect, allowing you to publish or interact with privileged content if you have the necessary permissions. **YOU SHOULD NEVER ADD YOUR API KEY TO YOUR SOURCE CODE!!!** An API key is like a password, and you should take the necessary steps to secure it. In Positron, we can add environment variables within our `Preferences: Open User Settings`. Below are the steps to create and save your Connect API Key.

1. Navigate to Posit Connect. Click your username in the top right corner and select **Manage You API Keys**.
2. Select **+New API Key**.
3. Name it whatever you like. `Workshop` is a good option.
4. Copy the new API key. **DON'T LOSE IT!** If you do lose it, you can create another one.
5. Open Posit Workbench and navigate to `Preferences: Open User Settings`.
6. Search for "Environment Variables: Set".
7. Click **Add Item**.
8. Add `CONNECT_API_KEY` as the key, and your API key as the value.
9. Once added, start a fresh Positron session!

You can check that your API key was added to your environment by running the following code:

```{r}
# Check if CONNECT_API_KEY is added to your environment
Sys.getenv("CONNECT_API_KEY")

# Also check if CONNECT_SERVER is in your environment
...
posit_cola_data <- read_csv(rawToChar(response$Body))
```
...
For modeling, you often split your data into a **training and testing** dataset.
...
Navigate to Posit Connect and check out your pinned model! You'll also notice a message asking you to create a **Model Card**. This is always a good idea, especially if you will be sharing your model with others.
...
# First register the Connect server
rsconnect::addServer(
...
vetiver_deploy_rsconnect(
  board = board, 
  name = paste0(username, "/posit_cola_revenue_model"), 
  account = username,
...
api_url <- "____" # Replace "____" with your API URL
...
pin_versions(board = board, name = paste0(username, "/posit_cola_revenue_model"))

# We can also extract some metadata from the pin
pin_meta(board = board, name = paste0(username, "/posit_cola_revenue_model"))$description
```

#### Create Another Model (Random forest)

Let's create another model to answer the same question as our logistic regression model: Can we predict posit-cola sales revenue? The code cell below will create, train, and assess the performance of a regression model using the random forest engine. This model will be used to compare against our first model.
...
# Get versions
versions_df <- pin_versions(board = board, name = paste0(username, "/posit_cola_revenue_model"))

# Get metadata
versions_with_meta <- versions_df |> 
  rowwise() |> 
  mutate(meta = pin_meta(
    board = board, 
    name = paste0(username, "/posit_cola_revenue_model"),
    version = version
  )$description) |> 
...
❗Be sure add in the version number as a string (eg. "536").

```{r}
#| label: monitor-model-performance

# Read in monitoring data
monitoring_data <- read_csv("posit_cola_sales_monitoring_data.csv")

# Load the vetiver model
vetiver_model_lm <- vetiver_pin_read(board, paste0(username, "/posit_cola_revenue_model"), version = "____")

# Load the vetiver model rf
vetiver_model_rf <- vetiver_pin_read(board, paste0(username, "/posit_cola_revenue_model"), version = "____")
...
# Combine metrics from both models for comparison
combined_metrics <- bind_rows(
  sales_revenue_metrics_lm |> mutate(model = "Linear Regression"),
  sales_revenue_metrics_rf |> mutate(model = "Random Forest")
)

# Create a combined plot showing both models
combined_metrics |>
  ggplot(aes(x = .index, y = .estimate, color = model)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 2) +
  facet_wrap(~.metric, scales = "free_y", ncol = 2) +
  scale_color_manual(values = c("Linear Regression" = "#4A90E2", "Random Forest" = "#E67E22")) +
  labs(
    title = "Model Performance Comparison Over Time",
    subtitle = "Posit Cola Revenue Models - Linear Regression vs Random Forest",
    x = "Date",
    y = "Metric Value",
    color = "Model Type"
  ) +
