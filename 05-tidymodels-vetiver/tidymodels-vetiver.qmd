...
# Create/Open your .Renviron file
usethis::edit_r_environ()

# If Positron does not open the file, use the command palette (Cmd/Ctrl + Shift + P) to "File: Open File"
#  and select the `.Renviron` file in your "home/username" directory.
```

Once you run the above command, you should see a new file open up in Positron. This is your `.Renviron` file and is usually located in your home (\~) directory. Before we set the variable, we first need to obtain an API key from Connect. Follow the steps below:

1.  Navigate to Posit Connect: https://pub.workshop.posit.team/.
...
Now you can close the `.Renviron` file and restart your R session (the refresh icon in your console) so that the API key variable get's added to our environment. To confirm it worked, try running the following code:
...
# Username
username <- "____" # Add in your username
```
