...
3.  Run the chunk by either clicking the green Play Button ▶︎ in the chunk's top right corner, or use the cmd+enter (Mac) / ctrl+enter (Windows) keyboard shortcut
...
if (Sys.getenv("POSIT_PRODUCT") == "CONNECT") {
  ...
  # Show that we are running in Posit Connect
  print("This code is running in Posit Connect!")

  # Create the Posit Connect client
  client <- connectapi::connect()
    
  # exchange the session token for the AWS credentials
  credentials <- connectapi::get_aws_content_credentials(client)
...
1.  Click the **Posit Publisher Icon** at the top of this document \> **Publish document with source code** \> enter a title \> press **Enter** to select `pub.workshop.posit.team` as the destination
2.  Select "Token Authentication" as the authentication method
