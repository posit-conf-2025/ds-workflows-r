---
title: "Create Coca-Cola Sales Dataset"
...
```{r business-logic}
coca_cola_sales <- coca_cola_sales |>
  ...
  mutate(
    ...
    product_multiplier = case_when(
      product == "Coca-Cola Classic" ~ runif(n(), 1.2, 1.8),
      product == "Diet Coke" ~ runif(n(), 0.8, 1.2),
      product == "Coke Zero" ~ runif(n(), 0.7, 1.1),
      product == "Sprite" ~ runif(n(), 0.9, 1.3),
      product == "Fanta Orange" ~ runif(n(), 0.6, 1.0),
      product == "Coca-Cola Energy" ~ runif(n(), 0.3, 0.7),
...
coca_cola_sales <- coca_cola_sales |>
  mutate(
    ...
    base_price = case_when(
      product == "Coca-Cola Classic" ~ runif(n(), 1.20, 1.80),
      product == "Diet Coke" ~ runif(n(), 1.25, 1.85),
      product == "Coke Zero" ~ runif(n(), 1.25, 1.85),
      product == "Sprite" ~ runif(n(), 1.15, 1.75),
      product == "Fanta Orange" ~ runif(n(), 1.10, 1.70),
      product == "Coca-Cola Energy" ~ runif(n(), 2.50, 3.50),
...
```{r save-clean}
# Save clean dataset
coca_cola_sales_clean <- coca_cola_sales
write.csv(coca_cola_sales_clean, "coca_cola_sales_data_clean.csv", row.names = FALSE)
...
coca_cola_sales_dirty <- coca_cola_sales_clean
```
...
missing_date_indices <- sample(
  nrow(coca_cola_sales_dirty),
  round(nrow(coca_cola_sales_dirty) * 0.02)
)
coca_cola_sales_dirty$date[missing_date_indices] <- NA

# 2. Missing promotion data (5% of records)
missing_promo_indices <- sample(
  nrow(coca_cola_sales_dirty),
  round(nrow(coca_cola_sales_dirty) * 0.05)
)
coca_cola_sales_dirty$promotion[missing_promo_indices] <- NA
```
...
negative_units_indices <- sample(
  nrow(coca_cola_sales_dirty),
  round(nrow(coca_cola_sales_dirty) * 0.005)
)
coca_cola_sales_dirty$units_sold[
  negative_units_indices
] <- -abs(coca_cola_sales_dirty$units_sold[negative_units_indices])

# 4. Extreme unit prices (pricing errors - 1% of records)
extreme_price_indices <- sample(
  nrow(coca_cola_sales_dirty),
  round(nrow(coca_cola_sales_dirty) * 0.01)
)
coca_cola_sales_dirty$unit_price[extreme_price_indices] <- sample(
...
revenue_error_indices <- sample(
  nrow(coca_cola_sales_dirty),
  round(nrow(coca_cola_sales_dirty) * 0.003)
)
coca_cola_sales_dirty$revenue[revenue_error_indices] <- coca_cola_sales_dirty$revenue[
...
cat("Clean Coca-Cola Sales Dataset:\n")
cat("Records:", nrow(coca_cola_sales_clean), "\n")
cat(
  "Date range:",
  as.character(min(coca_cola_sales_clean$date, na.rm = TRUE)),
  "to",
  as.character(max(coca_cola_sales_clean$date, na.rm = TRUE)),
  "\n"
)
cat("Products:", length(unique(coca_cola_sales_clean$product)), "\n")
cat("Regions:", length(unique(coca_cola_sales_clean$region)), "\n")
```
...
cat("\nDirty Coca-Cola Sales Dataset:\n")
cat("Records:", nrow(coca_cola_sales_dirty), "\n")
cat(
  "Date range:",
  as.character(min(coca_cola_sales_dirty$date, na.rm = TRUE)),
  "to",
  as.character(max(coca_cola_sales_dirty$date, na.rm = TRUE)),
  "\n"
)
cat("Products:", length(unique(coca_cola_sales_dirty$product)), "\n")
cat("Regions:", length(unique(coca_cola_sales_dirty$region)), "\n")
```
...
cat("Missing dates:", sum(is.na(coca_cola_sales_dirty$date)), "\n")
cat("Missing promotions:", sum(is.na(coca_cola_sales_dirty$promotion)), "\n")
cat(
  "Negative units sold:",
  sum(coca_cola_sales_dirty$units_sold < 0, na.rm = TRUE),
  "\n"
)
cat(
  "Extreme prices:",
  sum(
    coca_cola_sales_dirty$unit_price > 10 | coca_cola_sales_dirty$unit_price < 0.5,
    na.rm = TRUE
  ),
  "\n"
)
...
cat("Missing dates:", sum(is.na(coca_cola_sales_clean$date)), "\n")
cat("Missing promotions:", sum(is.na(coca_cola_sales_clean$promotion)), "\n")
cat(
  "Negative units sold:",
  sum(coca_cola_sales_clean$units_sold < 0, na.rm = TRUE),
  "\n"
)
cat(
  "Extreme prices:",
  sum(
    coca_cola_sales_clean$unit_price > 10 | coca_cola_sales_clean$unit_price < 0.5,
    na.rm = TRUE
  ),
  "\n"
)
...
write.csv(coca_cola_sales_dirty, "coca_cola_sales_data.csv", row.names = FALSE)
cat("\nDirty dataset saved as 'coca_cola_sales_data.csv'\n")
cat("Clean dataset already saved as 'coca_cola_sales_data_clean.csv'\n")
```

## Notes

This script generates two versions of the Coca-Cola sales dataset:

### Clean Dataset (`coca_cola_sales_data_clean.csv`)

-   Perfect data quality with no issues
-   Ready for analysis and modeling
-   Contains all business logic and patterns

### Dirty Dataset (`coca_cola_sales_data.csv`)
...
-   **Product mix**: Coca-Cola Classic dominates with 40% market share
